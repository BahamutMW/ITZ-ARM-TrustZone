...
namespace Kernel
{
	...
	constexpr Call_arg call_id_inject_vm_irq()          { return 38; }
	...
	/**
	 * Inject interrupt 'irq' into virtual machine 'vm' if possible
	 */
	inline void inject_vm_irq(Vm * const vm, unsigned const irq) {
		call(call_id_inject_vm_irq(), (Call_arg)vm, (Call_arg)irq); }
